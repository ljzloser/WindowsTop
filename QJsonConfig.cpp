...
bool QJsonConfig::init(const QJsonDocument& doc)
{
	return this->saveFile(doc.toJson());
}

bool QJsonConfig::read(const QVariant& key, QVariant& value)
{
	return this->read(QVariantList() << key, value);
}

bool QJsonConfig::read(const QVariantList& keys, QVariant& value)
{
	QJsonDocument doc;
	QJsonValue val;
...
QVariant QJsonConfig::read(const QVariant& key)
{
	QVariant var;
	this->read(key, var);
	return var;
...
QVariant QJsonConfig::read(const QVariantList& keys)
{
	QVariant var;
	this->read(keys, var);
	return var;
...
bool QJsonConfig::write(const QVariant& key, const QVariant& value, bool isAppend)
{
	return this->write(QVariantList() << key, value, isAppend);
}

bool QJsonConfig::write(const QVariantList& keys, const QVariant& value, bool isAppend)
{
...
void QJsonConfig::docToValue(QJsonDocument& doc, QJsonValue& val)
{
	if (doc.isObject())
...
QJsonDocument QJsonConfig::readJson()
{
	QJsonDocument doc;
	this->readJson(doc);
	return doc;
...
bool QJsonConfig::readJson(QJsonDocument& doc)
{
	QFile file;
	if (this->getFile(file, QJsonConfig::ReadOnly))
	{
